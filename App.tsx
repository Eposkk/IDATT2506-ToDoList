...
export default function App() {
  ...
  const addCategory = () => {
    ...
    if (newArray.length != 1) {
      flatListCategoryRef?.scrollToIndex({
        animated: true,
  ...
  const itemActivePress = (id: string) => {
    let newArray = list
    newArray[findCatgeoryIndex(selectedCategory)].taskList[findItemIndex(id)].status =
      !newArray[findCatgeoryIndex(selectedCategory)].taskList[findItemIndex(id)].status
    onChangeList(newArray)
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <View style={styles.modalButtonView}>
              <Pressable style={[styles.buttonModal, styles.buttonRed]} onPress={() => removeItem()}>
                <Text style={styles.text}>Delete</Text>
              </Pressable>
              <Pressable style={styles.buttonModal} onPress={() => setModalDeleteVisible(!modalDeleteVisible)}>
      ...
      >
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <TextInput style={styles.inputCatgeory} onChangeText={setCategoryName}></TextInput>
            <View style={styles.modalButtonView}>
              <Pressable style={[styles.buttonModal, styles.buttonGreen]} onPress={() => addCategory()}>
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={styles.modalText}>
              Are you sure you want to delete {list[findCatgeoryIndex(selectedCategory)]?.name}
            </Text>
            <View style={styles.modalButtonView}>
              <Pressable style={[styles.buttonModal, styles.buttonRed]} onPress={() => deleteCategory()}>
                <Text style={styles.text}>Delete</Text>
              </Pressable>
              <Pressable
                style={styles.buttonModal}
      ...
      <View style={styles.catgeoryContainer}>
        <FlatList
          ...
          renderItem={(item) => (
            <Pressable
              onPress={() => {
                setSelectedCategory(item.item.id)
                flatListCategoryRef?.scrollToIndex({
                  animated: true,
                  index: findCatgeoryIndex(item.item.id),
                  viewOffset: 80,
                })
              }}
            >
              <View
                style={
                  list[findCatgeoryIndex(selectedCategory)].id === item.item.id
                    ? styles.itemCategory
                    : styles.itemCategorySelected
                }
              >
        ...
        />
        <Pressable onPress={() => onAddCategoryPress()} style={styles.plussIcon}>
          <View>
        ...
        {list[findCatgeoryIndex(selectedCategory)]?.taskList.length == 0 && (
          <Pressable onPress={() => onCategoryRemovePress()}>
            <View style={styles.categoryRemoveContainer}>
              <Text>Press trash icon to delete category</Text>
